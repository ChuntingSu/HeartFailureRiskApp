...
st.warning("""
**⚠️ Clinical Disclaimer / Warning**

This tool is designed for educational and informational purposes only and is not a substitute for professional medical advice, diagnosis, or treatment. Scores are based on published clinical data and risk models and should be interpreted by a qualified healthcare professional. Do not use this calculator to make decisions about patient care without consulting a physician.
""")
st.markdown("---")
...
# 1. AHEAD Score Component Breakdown
st.subheader("📚 AHEAD Score Component Breakdown (Maximum 5 Points)")
st.markdown(f"""
This table shows the contribution of each sub-factor to the AHEAD Total Score.
| AHEAD Factor | Trigger Condition | Score Contribution |
| :--- | :--- | :--- |
| **Age** | $> 70$ Years | **{ahead_age_score} Points** |
| **Af (Atrial Fibrillation)** | Yes | **{af_score} Points** |
| **Hb (Hemoglobin)** | Male $< 13.0$ or Female $< 12.0$ g/dL | **{hgb_score} Points** |
| **HbA1C** | $\ge 6.5 \%$ | **{hba1c_score} Points** |
| **Cr (Creatinine)** | $> 1.47$ mg/dL | **{cr_score} Points** |
| **AHEAD Total** | **Sum of Above** | **{AHEAD_SCORE} Points** |
""")

# 2. Overall Score Component Breakdown
st.subheader("📊 Overall Risk Score Breakdown (Maximum 13 Points)")
st.markdown(f"""
This table shows the contribution of the five main risk areas to the final Overall Score.
| Risk Factor | Trigger Condition | Score Contribution |
| :--- | :--- | :--- |
| **Age** | $> 75$ Years | **{overall_age_score} Points** |
| **NYHA** | II or IV Classification | **{nyha_score} Points** |
| **AHEAD Total Threshold** | **AHEAD Score ({AHEAD_SCORE} Pts)** $> 1.5$ Points | **{ahead_overall_score} Points** |
| **eGFR** | $\le 56.45$ mL/min/1.73m² | **{egfr_score} Points** |
| **BMI** | $\le 22.5$ kg/m² | **{bmi_score} Points** |
| **OVERALL TOTAL** | **Sum of Above** | **{OVERALL_SCORE} Points** |
""")
